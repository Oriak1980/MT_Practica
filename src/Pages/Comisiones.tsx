...
export default function Comisiones() {
            ...
            id: 1, label: 'Recargas', content: <Grid container spacing={3} columns={12} direction={'row'} sx={{ m: 3, }}>
                {dataRecargas.map((c, i) => (
                    <Grid key={i} size={{ xs: 12, sm: 6, lg: 4 }} display={'flex'} justifyContent={'center'} alignItems={'center'}>
                        <Card variant="outlined" sx={{ backgroundColor: '#ffffff', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, p: 3, borderRadius: '9px', overflow: 'hidden', width: { xs: '100%', sm: '80%', md: '70%' }, maxWidth: 360, border: '2px solid #D04234' }}>
                            <Box component={'img'} src={c.image} alt='MainImage' sx={{ width: '100%', height: 'auto', borderRadius: 8, objectFit: 'cover' }} />
                            <Typography variant="h5" color="#D04234" fontFamily={'system-ui'} fontWeight={'bold'} sx={{ fontSize: { xs: '1rem', sm: '1.25rem' } }}>{c.id === 1.1 || c.id === 8.1 || c.id === 11.1 || c.id === 14 ? `$${c.number}` : `${c.number}%`}</Typography>
                            <Typography variant="h5" color="#000000" fontWeight={'bold'} fontFamily={'system-ui'} sx={{ fontSize: { xs: '1rem', sm: '1.1rem' }, textAlign: 'center' }}>{c.label}</Typography>
                        </Card>
                    </Grid>
                ))}
            </Grid>
    ...
    return (
        <>
            <Header />
            <Box display={'flex'} flexDirection={'column'} alignItems={'center'} mt={3} sx={{ width: '100%', px: 2 }}>
                <Typography variant="h3" fontFamily={'system-ui'} fontWeight={'bold'} color="#000000" sx={{ fontSize: { xs: '1.75rem', sm: '2.5rem' }, textAlign: 'center' }}>Comisiones</Typography>
                <Typography variant="h5" fontFamily={'system-ui'} fontWeight={'bold'} color="#828287" mt={2} sx={{ textAlign: 'center', maxWidth: '900px' }}>
                    Las comisiones mostradas son informativas y pueden actualizarse según las condiciones vigentes de cada servicio
                </Typography>
                <Card variant="outlined" sx={{ mt: 3, backgroundColor: 'rgba(130, 130, 135, 0.25)', p: 3, display: 'flex', flexDirection: 'row', width: '100%', maxWidth: '1700px', borderRadius: 8, justifyContent: 'center' }}>
                    <Typography variant="h5" fontFamily={'system-ui'} fontWeight={'bold'} color="#000000" sx={{ textAlign: 'center' }}>5 catalogos · 87 servicios</Typography>
                </Card>
                <Card variant="outlined" sx={{ mt: 3, backgroundColor: '#D04234', display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', maxWidth: '1700px', borderRadius: 8, p: 0.8, mb: 3 }}>
